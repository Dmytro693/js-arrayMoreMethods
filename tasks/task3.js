...
function addSuccessPercent(olympicRepresentation) {
  return olympicRepresentation.map(item => {
    let percentOfSuccess = '0%';  // За замовчуванням ставимо '0%' для таких випадків
    if (item.athletes > 0) {  // Перевіряємо, чи є спортсмени
      percentOfSuccess = ((item.medals / item.athletes) * 100).toFixed(1) + '%';
    }
    return { ...item, percentOfSuccess };  // Повертаємо новий об'єкт з відсотком
  });
}
